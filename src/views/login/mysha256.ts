const padding = (M: string): string => {
    let sizeLastBlock: number = 64;
    const sizeMLengthBuffer: number = 8;
    const sizeDivision: number = 1;
    const sizeMaxBlock: number = sizeLastBlock - sizeMLengthBuffer - sizeDivision;
    ...
    if (isOverflow) sizeLastBlock = 128;
  ...
  export const computeHash = (M: string): string => {
    ...
    for (let i = 0; i < dividedM.length; i++) {
      const Mi: RegExpMatchArray | null = dividedM[i].match(/.{8}/g);
